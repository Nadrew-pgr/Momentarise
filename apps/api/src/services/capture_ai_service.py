...
async def analyze_capture_with_sync_agent(
    *,
    ...
    system_prompt, _, _ = PromptComposer.compose(
        PromptComposerInput(
            ...
            extra_system_prompt=(
                ...
                "action_candidates must use types in: create_event|create_task|create_item|draft_reply|pay_invoice|summarize|review."
            ),
            user_timezone=(
                str(metadata.get("timezone")).strip()
                if isinstance(metadata, dict) and isinstance(metadata.get("timezone"), str)
                else None
            ),
            user_now=datetime.now(UTC),
            locale=locale,
            runtime_info={
                "agent": "capture_analysis",
                "mode": "analyze",
                "status": "processing",
