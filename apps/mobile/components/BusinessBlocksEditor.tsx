    ...
    <ScrollView className="flex-1" contentContainerClassName="gap-4 pb-28">
      {blocks.length === 0 ? (
        <View className="rounded-[28px] border border-dashed border-border bg-card px-5 py-10">
          <Text className="text-center text-sm leading-6 text-muted-foreground">
            {isRunMode ? "No content in this moment." : "Start from a starter kit or add a first block from the bottom actions."}
          </Text>
        </View>
      ) : null}
      ...
      {blocks.map((block, index) => {
        ...
        return (
          ...
          >
              ...
              <View className="gap-2">
                <View className="flex-row items-start justify-between gap-3">
                  <View className="flex-1">
                    <Text className="text-[9px] font-bold uppercase tracking-[2px] text-primary">{meta.eyebrow}</Text>
                    <View className="mt-0.5 flex-row items-center gap-2">
                      ...
                      <View className="rounded-full bg-muted/60 px-1.5 py-0.5">
                        <Text className="text-[9px] font-bold text-muted-foreground">#{index + 1}</Text>
                      </View>
                    </View>
                  </View>
                </View>
            ...
            {!isRunMode && (
              <View className="mt-3 flex-row items-center justify-between border-t border-border/50 pt-3">
                <View className="rounded-full bg-primary/10 px-2.5 py-0.5">
                  <Text className="text-[9px] font-black uppercase tracking-[1.5px] text-primary">{meta.title}</Text>
                </View>
            ...
            {isActive && !isRunMode && (
              <View className="mt-4 flex-row items-center justify-between rounded-2xl bg-muted/40 px-3 py-2">
                <Text className="flex-1 text-[10px] font-semibold text-muted-foreground mr-2">
                  Keep only essential fields visible. Use advanced for more.
                </Text>
                <Pressable
                  onPress={() => toggleAdvanced(block.id)}
            ...
            {isActive && !isRunMode && advancedBlockIds[block.id] && (
              <View className="mt-4 gap-2 rounded-2xl bg-muted/20 p-3">
                <Text className="text-[9px] font-black uppercase tracking-[1.5px] text-muted-foreground">Label</Text>
                <Input
                  value={block.label ?? ""}
                  onChangeText={(text) =>
                    patchBlock(index, (prev) => ({ ...prev, label: text || null }))
                  }
            ...
            ? {...prev, payload: {...prev.payload, text: text } }
            : prev
            )
                  }
            rows={isRunMode ? 4 : 8}
            className={cn(
              "rounded-[24px] bg-muted/30 px-4 py-3 text-base leading-relaxed",
              isRunMode ? "h-24" : "h-40"
            )}
            editable={editable}
                />
      ...
      {block.type === "checklist_block" ? (
        <View className="gap-4">
          <View className="flex-row items-center justify-between rounded-2xl bg-muted/20 px-4 py-3">
            <View>
              <Text className="text-[9px] font-black uppercase tracking-[1.5px] text-muted-foreground">Progress</Text>
          ...
          <View className="gap-3">
            {block.payload.items.map((item, itemIndex) => (
              <View key={item.id} className={cn("flex-row items-center gap-3 rounded-2xl border border-border/50 bg-muted/10 px-3", isRunMode ? "py-2" : "py-3")}>
                <Checkbox
                  checked={item.done}
      ...
      {block.type === "scale_block" ? (
        <View className="gap-3">
          <View className="flex-row items-center justify-between rounded-2xl bg-muted/20 px-4 py-3">
            <View>
              <Text className="text-[9px] font-black uppercase tracking-[1.5px] text-muted-foreground">Value</Text>
              <Text className="text-xl font-bold text-foreground">{block.payload.value}/{block.payload.max}</Text>
            </View>
            {block.payload.unit && (
              <View className="rounded-full bg-primary/10 px-2 py-0.5">
                <Text className="text-[10px] font-bold text-primary">{block.payload.unit}</Text>
              </View>
            )}
          </View>
          <View className="flex-row gap-1">
            {Array.from({ length: block.payload.max - block.payload.min + 1 }, (_, offset) => block.payload.min + offset).map((step) => (
              ...
              >
                <Text className={`text-[10px] font-bold ${step <= block.payload.value ? "text-primary-foreground" : "text-muted-foreground"}`}>{step}</Text>
              </Pressable>
            ))}
          </View>
      ...
      {block.type === "task_block" ? (
        <View className="gap-3">
          {!isRunMode ? (
            ...
            <View className="rounded-2xl bg-muted/20 px-4 py-3">
              <Text className="text-base font-bold text-foreground">{block.payload.title || "Untitled Task"}</Text>
            </View>
          )}
          <View className="flex-row gap-2">
            {[
              ...
              >
                <Text className={`text-center text-[10px] font-bold ${block.payload.status === option.value ? "text-primary-foreground" : "text-muted-foreground"}`}>{option.label}</Text>
              </Pressable>
            ))}
          </View>
        </View>
