...
export function useEventContent(eventId: string | null) {
  return useQuery<EventContentResponse | null>({
    ...
    queryFn: async () => {
      ...
      const res = await apiFetch(`/api/v1/events/${eventId}/content`);
      if (!res.ok) {
        throw new Error(await readApiError(res, "Failed to fetch moment content"));
      }
      const data = await res.json();
...
export function useUpdateEventContent(eventId: string | null) {
  ...
  return useMutation({
    mutationFn: async (payload: EventContentUpdateRequest) => {
      ...
      if (!res.ok) {
        throw new Error(await readApiError(res, "Failed to update moment content"));
      }
      const data = await res.json();
      return eventContentResponseSchema.parse(data) as EventContentResponse;
    },
...
export function useEventAnalytics(
  ...
  return useQuery<EventAnalyticsResponse | null>({
    ...
    queryFn: async () => {
      ...
      const res = await apiFetch(`/api/v1/events/${eventId}/analytics?${qs.toString()}`);
      if (!res.ok) {
        throw new Error(await readApiError(res, "Failed to fetch moment analytics"));
      }
      const data = await res.json();
